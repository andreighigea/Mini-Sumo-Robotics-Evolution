...
void SetupServos() {
	...
	RightServo.attach(SERVO_LEFT, 500, 2500); // attaches the servo on pin 18 to the servo object
	LeftServo.attach(SERVO_RIGHT, 500, 2500);
...
void FD(void *param){
	for (pos = 0; pos <= 110; pos += 1) { // goes from 0 degrees to 180 degrees
		LeftServo.write(90-pos);
		RightServo.write(pos);    // tell servo to go to position in variable 'pos'
		vTaskDelay(degreeDelay/portTICK_PERIOD_MS);             // waits 15ms for the servo to reach the position
	}
	taskHandle = NULL;
...
void FU(void *param){
	if (true){
		for (pos = 110; pos >= 0; pos -= 1) { // goes from 180 degrees to 0 degrees
			LeftServo.write(90-pos);
			RightServo.write(pos);    // tell servo to go to position in variable 'pos'
			vTaskDelay(10/portTICK_PERIOD_MS);             // waits 15ms for the servo to reach the position
		}
	}
	taskHandle = NULL;
	vTaskDelete(NULL); // Delete the task after completion
